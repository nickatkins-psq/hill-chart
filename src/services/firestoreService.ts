 ...
 * Always keeps the same project ID (slug) - only updates the name field
 * Returns the same project ID
 */
export async function updateProjectName(projectId: string, newName: string): Promise<string> {
  try {
    ...
    // Always keep the same project ID - just update the name field
